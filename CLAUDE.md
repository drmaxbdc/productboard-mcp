...
**`PRODUCTBOARD_OAUTH_CLIENT_SECRET`** is read at every token-endpoint call (initial exchange + each refresh) via `resolveClientSecret()` in [src/auth/types.ts](src/auth/types.ts). When set, included in `POST /oauth2/token` form body. Required for any app registered via PB's admin UI (which always issues a `client_secret` — there is no "Public Client" / PKCE-only option in that UI). Omitted from the request when undefined so the PKCE-only path still works once `POST /oauth2/register` (Public Client dynamic registration) comes back online upstream. The secret is **never persisted to `tokens.json`** — it is read from env at each request, so it stays in whatever store the deployment chose (typically the Claude Code MCP config env block, written by the consumer's own setup tooling). Deployments that distribute it this way should also set `PRODUCTBOARD_SETUP_HINT`, whose text is appended verbatim to credential errors so the user is told what to re-run. If the secret is missing while the embedded `DEFAULT_OAUTH_CLIENT_ID` is in use, `isMissingRequiredClientSecret()` records a deferred `setupError` and **no browser is opened** — the error surfaces on the first tool call. Note the browser otherwise opens at **MCP process start** (`createAuthResolution()` in `main()`), not on the first tool call.
